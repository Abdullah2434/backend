  ...
  try {
    ...
    try {
      ...
      if (!socialBuResult.success) {
        console.warn(
          `Failed to disconnect account ${accountIdNumber} from SocialBu:`,
          socialBuResult.message
        );
        // Continue with local disconnection even if SocialBu API fails
      } else {
        console.log(
          `Account ${accountIdNumber} successfully disconnected from SocialBu`
        );
      }
    } catch (socialBuError) {
      console.error(
        "Error calling SocialBu API to disconnect account:",
        socialBuError
      );
      // Continue with local disconnection even if SocialBu API fails
    }
    ...
    if (removeResult.success) {
      try {
        ...
        console.log(
          `Account ${accountIdNumber} deleted from UserConnectedAccount database for user ${userId}`
        );
      } catch (dbError) {
        console.error(
          "Error deleting account from UserConnectedAccount database:",
          dbError
        );
