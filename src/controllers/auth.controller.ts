...
export async function login(req: Request, res: Response) {
  try {
    ...
    return res.status(401).json({ success: false, message: e.message || 'Internal server error' })
  }
}
...
export async function updateProfile(req: Request, res: Response) {
  try {
    ...
    const updatedUser = await authService.updateProfile(currentUser._id.toString(), {
      firstName,
      lastName,
      phone
    })
...
export async function validateToken(req: Request, res: Response) {
  try {
    ...
    if (isResetToken && decoded?.userId) {
      ...
      if (userWithResetToken && (userWithResetToken as any).lastUsedResetToken === token) {
        return res.status(401).json({ success: false, message: 'Reset token has already been used' })
      }
    }
