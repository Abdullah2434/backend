...
export async function updatePaymentMethod(req: Request, res: Response) {
  try {
    ...
    console.log(
      `🔍 Confirming setup intent: ${setupIntentId} for user: ${payload.userId}`
    );
    ...
    const status = e.message.includes("Access token")
      ? 401
      : e.message.includes("not succeeded")
      ? 400
      : 500;
    return res.status(status).json({
      success: false,
...
export async function setDefaultPaymentMethod(req: Request, res: Response) {
  try {
    ...
    console.log(
      `🔍 Setting default payment method: ${paymentMethodId} for user: ${payload.userId}`
    );
...
export async function removePaymentMethod(req: Request, res: Response) {
  try {
    ...
    console.log(
      `🔍 Removing payment method: ${paymentMethodId} for user: ${payload.userId}`
    );
