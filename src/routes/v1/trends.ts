...
// Apply performance middleware to all trends routes
router.use(trendsCompression);
router.use(trendsCacheHeaders);
router.use(trendsPerformanceMonitoring);
...
router.get("/test-grok", async (req, res) => {
  try {
    ...
    const { generateRealEstateTrends } = await import(
