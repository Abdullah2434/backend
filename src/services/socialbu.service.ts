...
class SocialBuService {
   ...
   */
  async saveToken(tokenData: SocialBuLoginResponse): Promise<ISocialBuToken | null> {
    try {
      await connectMongo();
   ...
   */
  async connectAccount(provider: string, postbackUrl?: string, accountId?: string): Promise<SocialBuApiResponse> {
    try {
