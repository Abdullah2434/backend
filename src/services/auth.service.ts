...
import { sendVerificationEmail, sendPasswordResetEmail, sendResendVerificationEmail, sendWelcomeEmail } from './email'
import { 
  RegisterData, 
  LoginData, 
  ResetPasswordData, 
...
export class AuthService {
  ...
  generateResetToken(userId: string, email: string): string {
    return jwt.sign({ userId, email, type: 'reset' }, this.jwtSecret, { expiresIn: '15m' })
  }
  ...
  async resendVerificationEmail(email: string): Promise<{ message: string }> {
    ...
    await sendResendVerificationEmail(user.email, verificationToken, user.firstName)
