  ...
  middleware() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (this.isRateLimited(clientIP)) {
        ...
        const retryAfter = data ? Math.ceil((data.resetTime - Date.now()) / 1000) : 60
        
        console.log(`🔒 Rate limited: ${req.method} ${req.path} from ${clientIP}`)
...
export const registerRateLimiter = new ServerRateLimiter(rateLimitConfigs.register)
export const passwordResetRateLimiter = new ServerRateLimiter(rateLimitConfigs.passwordReset)
export const generalAuthRateLimiter = new ServerRateLimiter(rateLimitConfigs.general)
